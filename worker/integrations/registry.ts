...
// The only module that reads secret VALUES. Invariant: a live adapter is never built without complete credentials,
// whatever the database says, so removing a secret degrades to mock instead of failing.
...
export async function resolveAce(env: Env): Promise<Resolved<AceAdapter>> {
  ...
  const creds = credentials(env, 'ace')
  const mode = effectiveMode(row, creds)
  const adapter =
    mode === 'live'
      ? createAceLiveAdapter(createHttpClient({ provider: 'ace', baseUrl: creds.baseUrl!, headers: () => ({ 'x-api-key': env.ACE_API_KEY! }) }))
...
export async function resolveInttra(env: Env): Promise<Resolved<InttraAdapter>> {
  ...
  const creds = credentials(env, 'inttra')
  const mode = effectiveMode(row, creds)
  const adapter =
    mode === 'live'
      ? createInttraLiveAdapter({ baseUrl: creds.baseUrl!, clientId: env.INTTRA_CLIENT_ID!, apiKey: env.INTTRA_API_KEY! })
